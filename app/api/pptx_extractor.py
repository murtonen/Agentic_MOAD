...
class PPTXExtractor:
    ...
    def extract_content_with_metadata(self) -> List[Dict[str, Any]]:
        """
        Extract text content with metadata from all slides.
        
        Returns:
            List of dictionaries with slide content and metadata
        """
        if not os.path.exists(self.file_path):
            raise FileNotFoundError(f"PowerPoint file not found: {self.file_path}")
